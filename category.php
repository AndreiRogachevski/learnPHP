...
<div class="row">
  <div class="col-md-9">
    ...
    <h1 class="display-4 border-bottom pb-2"><?= get_category_title($category_id) ?>:</h1>
    <?php foreach ($posts as $post) : ?>
      <div class="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
        ...
        <div class="col p-4 d-flex flex-column position-static">
          <h3 class="mb-0"><a href="/post.php?post_id=<?= $post['id'] ?>"><?= $post['title'] ?></a></h4>
            <p class="card-text d-block"><?= substr($post['content'], 0, 128) . '...'  ?>
              <a href="/post.php?post_id=<?= $post['id'] ?>" class="mb-auto d-block">Продолжить читать</a>
            </p>
            <ul class="text-muted nav gap-2">
              <li class='nav-item d-flex align-items-center'><i class='bi bi-list-ul me-1'></i><a href="/category.php?id=<?=$post['category_id']?>" class="text-decoration-none position-relative z-1"><?= get_category_title($post['category_id']) ?></a>| </li>
